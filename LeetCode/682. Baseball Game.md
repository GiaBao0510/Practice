...
public static int CalPoints(string[] operations) {
	...
	if(operations.Length == 0 || operations == null) return 0;
	...
	foreach(string op in operations) {
		...
		switch(op){
			case "C":
				if(record.Count > 0){
					sum -= record.Pop(); //update sum when removing the first element
				}
				break;
